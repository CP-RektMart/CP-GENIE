...
@router.post("/contextual/{rag_type}/{session_id}")
async def contextual_chat(
    rag_type: str,
    session_id: str,
    chatrequest: ChatRequest,
    retriever_fac: str = Depends(get_retriever_fac_reranked),
    retriever_oth: str = Depends(get_retriever_oth_reranked),
    llm: str = Depends(get_llm),
) -> ChatResponse:

    if rag_type not in RAG_CLASSES_CONTEXTUAL:
        raise ValueError(f"Invalid rag_type: {rag_type}")
    if not session_id:
        raise ValueError("session_id is required")
    if not chatrequest.query:
    ...
    rag_class = RAG_CLASSES_CONTEXTUAL[rag_type]
    memory = get_by_session_id(session_id)
